...
function conversion(time){
  ...
  // var  _date = new Date('2016-08-16 15:38:52');
   var  _date = new Date(time);

    //Tue Aug 16 2016 15:38:52 GMT+0530 (India Standard Time)
...
function conversiondate(time){
    ...
    // var  _date = new Date('2016-08-16 15:38:52');
     var  _date = new Date(time);
      ...
      var Denmark_Time = new Date(milliSeconds - (270*60*1000));
  var month=parseInt( Denmark_Time.getMonth()+1);
var date_mark= Denmark_Time.getDate();
  if ( month < 10 ) {
    month= "0" + month;
} else {
    month= month;
...
if ( date_mark < 10 ) {
    date_mark= "0" + date_mark;
} else {
    date_mark= date_mark;
}
    var Finally_denmark_date=  Denmark_Time.getFullYear()+"-"+month+"-"+ date_mark;
  
    return Finally_denmark_date;
  }
                    ...
                    switch (keys[0]) 
					{
                        case "New":
							...
							//	var res = datesplit[0]+'-'+datesplit[1]+'<i class="madhu">-'+datesplit[2]+'</i>'+'<i class="madhu">-'+str2+'</i>';
								//praveen
								 if(localStorage.getItem('locationbasednamevendor')!='India'){
                                    var split_date_denmark= datesplit[2]+'-'+datesplit[1]+'-'+datesplit[0];
                                    var time_conversion= split_date_denmark+" "+str2; 
                                    time_val= conversion(time_conversion);
                                    var denmark_date=   conversiondate(time_conversion)
                                    var split_date=denmark_date.split("-");
									 	var res = split_date[2]+'-'+split_date[1]+'-'+split_date[0].slice(2)+'<i class="madhu">-'+datesplit[2]+'</i>'+'<i class="madhu">-'+str2+'</i>';
   ...
   $.ajax(
    {
        ...
        success: function(response)
        {
            ...
            if (response.orderprintsummmary.length > 0)
            {
				...
				var res = date_of_order.split('-')
               // $('#dateoforder').text(date_of_order);
					 if(localStorage.getItem('locationbasednamevendor')!='India'){
                $('#dateoforder').text(res[0]+'-'+res[1]+'-'+res[2].slice(2));
					 }
					 else
					 {
						    $('#dateoforder').text(date_of_order);
